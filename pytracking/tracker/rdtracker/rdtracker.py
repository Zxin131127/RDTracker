...
class RDTracker(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        self.debug_info['seq_name'] = info['seq_name']
        self.debug_info['results_dir'] = info['results_dir']
        ...
        if update_flag and self.params.get('update_classifier', False):
            ...
            if (self.frame_num - 1) % self.params.transformer_skipping == 0:
                ...
                if self.x_clf.shape[0] < self.params.transformer_memory_size:
                    self.transformer_label = torch.cat([cur_tf_label.unsqueeze(1).cuda(),
                                                        self.transformer_label],
                                                       dim=0)
                    self.x_clf = torch.cat([x_clf, self.x_clf], dim=0)
                    transformer_memory_thistime, _ = self.net.classifier.transformer.encoder(x_clf.unsqueeze(1), pos=None)  
                    self.transformer_memory = torch.cat([transformer_memory_thistime, self.transformer_memory], dim=0)


                else:
                    self.transformer_label = torch.cat([cur_tf_label.unsqueeze(1).cuda(),
                                                        self.transformer_label[:-1, ...]],
                                                       dim=0)  # delete the end list
                    self.x_clf = torch.cat([x_clf, self.x_clf[:-1, ...]], dim=0)
                    transformer_memory_thistime, _ = self.net.classifier.transformer.encoder(x_clf.unsqueeze(1), pos=None) 
                    self.transformer_memory = torch.cat([transformer_memory_thistime,
                                                         self.transformer_memory[:-x_clf.shape[-1]*x_clf.shape[-2], ...]], dim=0)
        ...
        if self.visdom is not None:
            self.visdom_draw_tracking(image, output_state)

            plt.matshow(score_map.squeeze().flip(0).cpu().detach().numpy())
            plot_i = self.debug_info['results_dir'] + '/confidence_map/' + \
                     self.debug_info['seq_name']
            if not os.path.exists(plot_i):
                os.makedirs(plot_i)
            plt.savefig(plot_i + '/' + str(self.debug_info['frame_num']) + '_' + \
                        str(round(self.debug_info['max_score'], 2)) + '.pdf')
            plt.close()
    ...
    def transformer_decoder(self, sample_x: TensorList):
        """Transformer."""
        with torch.no_grad():
            decoded_feat, out_feat = self.net.classifier.transformer.decoder(sample_x.unsqueeze(0), memory=self.transformer_memory,
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        self.transformer_memory_first = self.transformer_memory
    ...
    def visdom_draw_tracking(self, image, box, segmentation=None):
        if hasattr(self, 'search_area_box'):
            self.visdom.register((image, box, self.search_area_box,
                                  self.debug_info['results_dir'], self.debug_info['seq_name'], str(self.debug_info['frame_num'])),
                                  'Tracking', 1, 'Tracking')
        else:
            self.visdom.register((image, box, self.search_area_box,
                                  self.debug_info['results_dir'], self.debug_info['seq_name'], str(self.debug_info['frame_num'])),
