...
# from TransformerTrack.pytracking.evaluation import Sequence, Tracker
# from TransformerTrack.ltr.data.image_loader import imwrite_indexed
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, visdom_info=None, net_path=''):
    ...
    if debug:
        output = tracker.run_sequence(seq, debug=debug, visdom_info=visdom_info, net_path=net_path)
    else:
        try:
            output = tracker.run_sequence(seq, debug=debug, visdom_info=visdom_info, net_path=net_path)
...
def run_dataset(dataset, trackers, net_path, debug=False, threads=0, visdom_info=None):
    ...
    if mode == 'sequential':
        for seq in dataset:
            for tracker_info in trackers:
                run_sequence(seq, tracker_info, debug=debug, visdom_info=visdom_info, net_path=net_path)
    elif mode == 'parallel':
