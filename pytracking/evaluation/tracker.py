...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, run_id: int = None, display_name: str = None):
        ...
        if self.run_id is None:
            ...
            self.results_dir = '{}'.format(env.results_path)
    ...
    def run_sequence(self, seq, visualization=None, debug=None,
                     visdom_info=None, multiobj_mode=None, net_path='',
                     qxm=None):
        ...
        """
        params = self.get_parameters(net_path)
        ...
        tracker.optimize_filter = self.optimize_filter
        tracker.cascade_level = self.cascade_level
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            info['results_dir'] = self.results_dir
    ...
    def get_parameters(self, net_path):
        """Get parameters."""
        param_module = importlib.import_module('pytracking.parameter.{}.{}'.format(self.name, self.parameter_name))
        params = param_module.parameters()
        if self.name != 'eco':
            params.net = NetWithBackbone(net_path=net_path, use_gpu=params.use_gpu, optimize_filter=self.optimize_filter)
        else:
            from pytracking.features import deep
            from pytracking.features.extractor import MultiResolutionExtractor

            features = deep.ResNet18m1(output_layers=['vggconv1', 'layer3'], net_path=net_path, use_gpu=params.use_gpu,
