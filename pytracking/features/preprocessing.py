...
def sample_patch_multiscale(im, pos, scales, image_sz, mode: str='replicate', max_scale_change=None, plt_visdom = None):
    ...
    patch_iter, coord_iter = zip(*(sample_patch(im, pos, s*image_sz, image_sz, mode=mode,
                                                ...
                                                plt_visdom=plt_visdom) for s in scales))
...
def sample_patch(im: torch.Tensor, pos: torch.Tensor, sample_sz: torch.Tensor, output_sz: torch.Tensor = None,
                 mode: str = 'replicate', max_scale_change=None, plt_visdom = None, is_mask=False):
    ...
    if df > 1:
        ...
        posl = torch.floor_divide(posl - os, df)     # new position
