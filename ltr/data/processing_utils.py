...
def sample_target_adaptive(im, target_bb, search_area_factor, output_sz, mode: str = 'replicate',
    ...
    if mode == 'inside' or mode == 'inside_major':
        ...
        if crop_sz_y < 1 or crop_sz_y <1 or im_w <1or im_h <1:
            print('crop: {},{}, im: {},{}'.format(crop_sz_x, crop_sz_y, im_w, im_h))
...
def gaussian_label_function(target_bb, sigma_factor, kernel_sz, feat_sz, image_sz, end_pad_if_even=True, density=False,
    ...
    # folder = '/data/zxin/MViT_STCF/pytracking/input_images'
    # # if density is False:
    # #     folder = '{}/desity_False'.format(folder_mask)
    # # else:
    # #     folder = '{}/desity_True'.format(folder_mask)
    #
    # for aa in range(label.shape[0]):
    #     # plot_single_pdf_3d(label[aa], label.shape, '{}/3Dmask_{}.pdf'.format(folder, aa+1))
    #     # tt = label[aa]
    #     plot_single_pdf(label[aa], '{}/mask_{}.pdf'.format(folder, aa+1))
    # plt.matshow(label[0])


    return label

def plot_single_pdf_3d(x, shape, pdf_name):
    x1 = np.arange(0, shape[-2], 1)
    y1 = np.arange(0, shape[-1], 1)
    x1,y1 = np.meshgrid(x1,y1)

    fig = plt.figure()
    ax = fig.gca(projection = '3d')
    ax.plot_surface(x1, y1, x.cpu().detach().numpy(), cmap=plt.get_cmap('rainbow'))
    plt.savefig(pdf_name)
    plt.close()

def plot_single_pdf(x, pdf_name):
    plt.matshow(x.cpu().detach().numpy())
    plt.savefig(pdf_name)
    plt.close()
